...
class Program:
    def __init__(self, data):
        self.pid = data[0]
        self.arrival = data[1]
        self.instruct_count = data[2]
        self.executed = 0

    def execute(self, n_time_unit):
        self.executed += n_time_unit
        return self.instruct_count - self.executed
    
    def get_turnaround(self, complete_time):
        return complete_time - self.arrival
...
class ScheduleEnv(gym.Env):
    ...
    def __init__(self, data, encoder_context, max_priority):
        # initialize env with given information
        self.data = data
        self.encoder_context = encoder_context
        self.max_priority = max_priority

        self.observation_space = None
        self.action_space = spaces.Discrete(max_priority, start=1)
    ...
    def _get_next_program(self):
        while self.current_time < self.data[self.counter,1]:
            self.counter += 1
        self.counter = self
        
    def _update(self):
        n_cycle = self.data[self.counter, 1] - self.current_time
        if n_cycle > 0:
    ...
    def reset(self, seed=None, options=None):
        ...
        self.priority_queue.put((1, self.data[0]))
        self.counter += 1
        self._update()

        info = self._get_info()

        return self.priority_queue, info
